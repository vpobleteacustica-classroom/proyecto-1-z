...
def extract_melody(vocals_path: str,
    ...
    """
    Extrae la melodía de vocals.wav usando CREPE en modo rápido.
    Devuelve lista de eventos (pitch_midi, start, end).
    """
    ...
    # Parámetros crepe
    step_size = crepe_step_ms  # ms por frame
    model_capacity = 'tiny' if use_tiny else 'full'

    # crepe.predict espera señal completa; si es muy larga, procesamos por bloques de 60s
    events = []
    block_sec = 60.0
    ...
    # Post-process: fusionar eventos cortos (ruido)
    merged = []
    min_dur = 0.06
    for e in events:
        p, s, t = e
        if not merged:
            ...
            last = merged[-1]
            if last[0] == p and s - last[2] <= 0.05:
                # extender
                last[2] = t
            else:
                merged.append([p, s, t])
    # filtrar duraciones muy pequeñas
    final = []
    for p,s,t in merged:
        if t - s >= min_dur:
...
# --------------------------
def run_pipeline(song_dir: str, out_dir: str = "arrangement_out_v2"):
    ...
    chords = chroma_chords([guitar, other], window_sec=0.6)
    print(f"    Acordes detectados: {len(chords)}")

    print("3) Construyendo arreglo (voicings realistas + patrón pop)...")
    pm = build_arrangement_v2(melody, chords, style='pop')
